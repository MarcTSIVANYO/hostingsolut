...
function king_get_whois_server(){
    global $king;
	...
	$servers = array(
		...
		"global" 	=> array("whois.nic.global","Domain not found."),
		"globo" 	=> array("whois.gtlds.nic.br","Domain not found."),
		...
		"mil" 	=> array("whois.nic.mil","Status: Not Registered"),
    ...
    if( isset($king->cfg['whois_servers']) && $king->cfg['whois_servers'] != ''){
        $extra_list = array();
        $extra = $king->cfg['whois_servers'];
        $extra = explode("\n", $extra);

        foreach( $extra as $item){
            $obj = explode('|', $item);
            $extra_list[ $obj[0] ] = array( $obj[1], $obj[2]);
        }

        $rservers = array_merge($servers, $extra_list);
    }else{
        $rservers = $servers;
    }
